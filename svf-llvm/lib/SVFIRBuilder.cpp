 ...
 */
bool SVFIRBuilder::computeGepOffset(const User *V, LocationSet& ls, bool& vGep)
{
    assert(V);
...
void SVFIRBuilder::processCE(const Value* val)
{
    if (const Constant* ref = SVFUtil::dyn_cast<Constant>(val))
    {
        if (const ConstantExpr* gepce = isGepConstantExpr(ref))
        {
            ...
            LocationSet ls;
            bool vGep = false;
            bool constGep = computeGepOffset(gepce, ls, vGep);
             ...
             */
            addGepEdge(pag->getValueNode(LLVMModuleSet::getLLVMModuleSet()->getSVFValue(opnd)), pag->getValueNode(LLVMModuleSet::getLLVMModuleSet()->getSVFValue(gepce)), ls, constGep, vGep);
...
void SVFIRBuilder::visitGetElementPtrInst(GetElementPtrInst &inst)
{
    ...
    LocationSet ls;
    bool vGep = false;
    bool constGep = computeGepOffset(&inst, ls, vGep);
    addGepEdge(src, dst, ls, constGep, vGep);
}
                ...
                for (ExtAPI::Operation op : allOperations)
                {
                    ...
                    else if (op.getOperator() == "GepStmt")
                    {
                        if (op.getOperands().size() == 3)
                        {
                            ...
                            addNormalGepEdge(op.getOperands()[0], op.getOperands()[1], ls, false);
                        }
                        else
...
NodeID SVFIRBuilder::getGepValVar(const Value* val, const LocationSet& ls, const SVFType* elementType)
{
    ...
    if (gepval==UINT_MAX)
    {
        ...
        addGepEdge(base, gepNode, ls, true, false);
        setCurrentLocation(cval, cbb);
        return gepNode;
    }
    else
